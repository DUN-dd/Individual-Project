...
The central runner is \texttt{all\_tests\_runner.gd}, which recursively discovers every \texttt{test\_*.gd} file under three roots: \texttt{Unit Test/}, \texttt{src/scripts/tests/}, and \texttt{src/scripts/ui/}. Five files are excluded from auto-discovery because they are runner scripts themselves or require separate standalone execution. Discovery produces approximately 58 suites; these are executed sequentially, each with a 60-second watchdog that records a timeout failure if a suite hangs. Before running the discovered suites, the runner executes a built-in inline suite of ``Quick Sanity Checks'' that verifies the four most critical autoloads: \texttt{ServiceLocator}, \texttt{ErrorReporter}, \texttt{GameState}, and \texttt{AIManager}. Two additional scripts (\texttt{test\_achievement\_system\_api.gd} and \texttt{test\_providers\_mock.gd}) are executed as standalone headless processes after the main runner finishes, giving a total of approximately 61 suites in a complete CI run.
