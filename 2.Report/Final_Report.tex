...
All prompts and persona strings support bilingual generation: language selection (English or Chinese) is passed through the AI call chain and resolved in \texttt{AIPromptsI18n}.
...
The FSM 30-Day Rebirth Challenge is a fully implemented game mode, accessible from the main menu, that reframes the core loop as a structured 30-day self-improvement programme endorsed by the Flying Spaghetti Monster. Players commit to completing one daily mission for 30 consecutive days, each day presenting a themed content card (backed by per-day assets in \texttt{src/assets/rebirth\_challenge/}, days 1--8 illustrated). The mode is managed by the \texttt{FSMChallengeModule} class, which tracks \texttt{challenge\_start\_date}, \texttt{current\_day}, \texttt{days\_completed}, and login continuity.

The central satirical mechanic is an inevitable, scripted crash: the module enforces \texttt{DAYS\_BEFORE\_CRASH = 8}, meaning the challenge always catastrophically collapses on day 8 regardless of player performance. Daily themes (Days 1--8, keyed in \texttt{FSMDailyContentData}) escalate in toxic-positivity intensity before the crash triggers \texttt{challenge\_crash\_triggered} and locks further progress. A \texttt{FSMRebirthExplanation} scene then presents an ironic post-mortem. This design deliberately subverts the real-world ``30-day challenge'' genre, embodying the game's central thesis that manufactured positivity is structurally self-defeating. The constant \texttt{TOTAL\_DAYS = 30} remains inaccessible by design.
