...
func _test_basics() -> void:
	...
	assert(AudioManager.has_sound("gloria/zh/gloria_guilt_03"), "Missing zh Gloria WAV sound (gloria_guilt_03)")
...
func _test_volume_clamping() -> void:
	...
	var s := AudioManager.get_volume_settings()
	assert(abs((s.master_volume / 100.0) - 1.0) <= TOL, "Master volume not clamped to 1.0")
	var master_idx = AudioServer.get_bus_index("Master")
	if master_idx != -1:
		var actual_db = AudioServer.get_bus_volume_db(master_idx)
		var expected_db = linear_to_db(1.0)
		assert(abs(actual_db - expected_db) <= TOL, "Master bus DB not clamped properly at max")
	AudioManager.set_master_volume(-0.5)
	s = AudioManager.get_volume_settings()
	assert(abs((s.master_volume / 100.0) - 0.0) <= TOL, "Master volume not clamped to 0.0")
	if master_idx != -1:
		var actual_db = AudioServer.get_bus_volume_db(master_idx)
		assert(actual_db <= -60.0 or actual_db <= linear_to_db(0.001), "Master bus DB not clamped properly at min")
...
func _test_non_blocking_under_load() -> void:
	...
	await get_tree().process_frame
	await timer.timeout
	assert(_timer_fired, "Timer blocked by audio activity")
