...
func _test_bundled_story_pages_complete() -> void:
	...
	assert(pages.size() >= IntroStoryScript.TOTAL_PAGES, "Bundled intro story data should include all intro pages")
	for i in range(IntroStoryScript.TOTAL_PAGES):
		...
		assert(not text_zh.is_empty(), "Bundled zh story text should not be empty on page %d" % (i + 1))
		assert(not text_en.is_empty(), "Bundled en story text should not be empty on page %d" % (i + 1))
		assert(not text_zh.contains(LocalizationManager.get_translation("TEST_INTRO_PLACEHOLDER_ZH", "zh") if LocalizationManager else "placeholder"), "Bundled zh story text should not be placeholder on page %d" % (i + 1))
		assert(not text_en.contains("Placeholder"), "Bundled en story text should not be placeholder on page %d" % (i + 1))
...
func _test_csv_story_pages_complete() -> void:
	...
	assert(file != null, "Intro story CSV should be loadable in source tree")
	if file == null:
		return
	var row_index := 0
	var valid_rows := 0
	while not file.eof_reached():
		...
		valid_rows += 1
	file.close()
	assert(valid_rows >= IntroStoryScript.TOTAL_PAGES, "CSV intro story should provide complete data for all intro pages")
