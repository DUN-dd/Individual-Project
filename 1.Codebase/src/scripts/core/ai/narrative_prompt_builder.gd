...
static func build_consequence_prompt(choice: Dictionary, success: bool, lang: String, force_complete: bool = false) -> String:
	...
	var result := _build_prompt_with_skill("consequence-generation", lang, context_parts)
	if force_complete:
		var force_skill_content := _load_skill_content("force-mission-complete", lang)
		if not force_skill_content.strip_edges().is_empty():
			result += "\n\n" + force_skill_content.strip_edges()
	return result
...
static func _build_json_schema(lang: String) -> String:
	var cautious_preview_label := _s_tr("ARCHETYPE_LABEL_CAUTIOUS", lang)
	if lang.to_lower().begins_with("de"):
		cautious_preview_label = "[Vorsichtig]"
	var preview_labels := {
...
static func _load_skill_content(skill_name: String, lang: String) -> String:
	var skill_mgr := _get_skill_manager()
	if not skill_mgr:
		push_error("[NarrativePromptBuilder] SKILL LOAD FAILED: SkillManager not found for skill='%s', lang='%s'" % [skill_name, lang])
		return ""
	if not skill_mgr.is_initialized():
		push_error("[NarrativePromptBuilder] SKILL LOAD FAILED: SkillManager not initialized for skill='%s', lang='%s'" % [skill_name, lang])
		return ""
	var content: String = skill_mgr.load_skill(skill_name, lang)
	if content.is_empty():
		push_error("[NarrativePromptBuilder] SKILL LOAD FAILED: Empty content for skill='%s', lang='%s'" % [skill_name, lang])
	return content
static func _build_prompt_with_skill(skill_name: String, lang: String, context_parts: Array[String], replacements: Dictionary = {}) -> String:
	var skill_mgr := _get_skill_manager()
	if not skill_mgr:
		push_error("[NarrativePromptBuilder] PROMPT BUILD FAILED: SkillManager not found for skill='%s', lang='%s'" % [skill_name, lang])
		return ""
	if not skill_mgr.is_initialized():
		push_error("[NarrativePromptBuilder] PROMPT BUILD FAILED: SkillManager not initialized for skill='%s', lang='%s'" % [skill_name, lang])
		return ""
	var skill_content: String = skill_mgr.load_skill(skill_name, lang)
	if skill_content.is_empty():
		push_error("[NarrativePromptBuilder] PROMPT BUILD FAILED: Empty skill content for skill='%s', lang='%s'. The AI prompt will be incomplete!" % [skill_name, lang])
		return ""
