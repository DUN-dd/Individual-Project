...
const FORENSIC_EASTER_EGG_URL := "https://www.youtube.com/watch?v=NP6S1osAYtw"
const FORENSIC_CLICK_TARGET := 5
const FORENSIC_CLICK_TIMEOUT := 5.0
...
var _forensic_click_count: int = 0
var _forensic_click_timer: float = 0.0
var _forensic_label: Label = null
...
func _process(delta: float) -> void:
	if _forensic_click_count > 0:
		_forensic_click_timer += delta
		if _forensic_click_timer >= FORENSIC_CLICK_TIMEOUT:
			_forensic_click_count = 0
			_forensic_click_timer = 0.0
func _setup_forensic_easter_egg() -> void:
	if not _subtitle_label:
		return
	var parent_container = _subtitle_label.get_parent()
	if not parent_container:
		return
	_forensic_label = Label.new()
	_forensic_label.text = _tr("EASTER_EGG_FORENSIC_TEXT")
	_forensic_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_forensic_label.add_theme_font_size_override("font_size", 11)
	_forensic_label.add_theme_color_override("font_color", Color(0.55, 0.75, 0.92, 0.60))
	_forensic_label.mouse_filter = Control.MOUSE_FILTER_STOP
	_forensic_label.mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
	_forensic_label.tooltip_text = _tr("EASTER_EGG_FORENSIC_HINT").replace("{remaining}", str(FORENSIC_CLICK_TARGET))
	_forensic_label.gui_input.connect(_on_forensic_label_gui_input)
	parent_container.add_child(_forensic_label)
func _on_forensic_label_gui_input(event: InputEvent) -> void:
	if not (event is InputEventMouseButton):
		return
	var mb := event as InputEventMouseButton
	if mb.button_index != MOUSE_BUTTON_LEFT or not mb.pressed:
		return
	_forensic_click_count += 1
	_forensic_click_timer = 0.0
	var remaining := FORENSIC_CLICK_TARGET - _forensic_click_count
	if _forensic_label and remaining > 0:
		_forensic_label.tooltip_text = _tr("EASTER_EGG_FORENSIC_CLICK").replace("{remaining}", str(remaining))
	if _forensic_click_count >= FORENSIC_CLICK_TARGET:
		_forensic_click_count = 0
		_forensic_click_timer = 0.0
		_show_forensic_popup()
func _show_forensic_popup() -> void:
	...
	panel_style.border_color = Color(0.40, 0.65, 0.85, 0.60)
	...
	margin.add_child(vbox)
	var title_lbl = Label.new()
	title_lbl.text = _tr("EASTER_EGG_FORENSIC_TITLE")
	title_lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	title_lbl.add_theme_font_size_override("font_size", 17)
	title_lbl.add_theme_color_override("font_color", Color(0.75, 0.90, 1.00))
	...
	body_lbl.text = _tr("EASTER_EGG_FORENSIC_BODY")
	body_lbl.add_theme_font_size_override("normal_font_size", 14)
	body_lbl.add_theme_color_override("default_color", Color(0.88, 0.91, 0.97))
	vbox.add_child(body_lbl)
	var listen_btn = Button.new()
	listen_btn.text = _tr("EASTER_EGG_FORENSIC_LISTEN")
	UIStyleManager.apply_button_style(listen_btn, "primary", "medium")
	listen_btn.pressed.connect(func(): OS.shell_open(FORENSIC_EASTER_EGG_URL))
	vbox.add_child(listen_btn)
	var close_btn = Button.new()
	close_btn.text = _tr("EASTER_EGG_FORENSIC_CLICK").replace("{remaining}", "0")
