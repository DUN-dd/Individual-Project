...
func _build_story_summary_prompt(story_text: String) -> String:
	...
	lines.append(_tr("JOURNAL_AI_SUMMARY_ROLE"))
	lines.append(_tr("JOURNAL_AI_SUMMARY_INSTRUCTION"))
	lines.append(_tr("JOURNAL_AI_SUMMARY_FORMAT"))
	lines.append("")
	lines.append(_tr("JOURNAL_AI_SUMMARY_STORY_LABEL"))
	lines.append(truncated)
	lines.append("")
	lines.append(_tr("JOURNAL_AI_SUMMARY_OUTPUT_LABEL"))
	return "\n".join(lines)
...
func _extract_clean_story_text(raw_text: String) -> String:
	...
	if trimmed.is_empty():
		return ""
	if _looks_like_raw_json(trimmed):
		var extracted := _try_extract_story_from_json(trimmed)
		if not extracted.is_empty():
			return extracted
	var clean := _remove_json_fragments(trimmed)
	return clean.strip_edges()
func _looks_like_raw_json(text: String) -> bool:
	...
	if text.find("\"storyText\":") >= 0:
		return true
	return false
func _try_extract_story_from_json(text: String) -> String:
	...
	return ""
func _find_json_string_end(text: String, start: int) -> int:
	var i := start
	while i < text.length():
		if text[i] == "\\":
			i += 2
		...
		i += 1
	return -1
func _remove_json_fragments(text: String) -> String:
	var result := text
	var patterns := [
		RegEx.new(),
		RegEx.new(),
...
func _build_guardrail_lines(lang: String) -> Array[String]:
	var lines: Array[String] = []
	lines.append(_tr("JOURNAL_AI_SUGGESTION_JSON"))
	lines.append(_tr("JOURNAL_AI_SUGGESTION_COUNT") % MAX_SUGGESTIONS)
	lines.append(_tr("JOURNAL_AI_SUGGESTION_LIMIT") % SUGGESTION_WORD_LIMIT)
	lines.append(_tr("JOURNAL_AI_SUGGESTION_TONE"))
	return lines
