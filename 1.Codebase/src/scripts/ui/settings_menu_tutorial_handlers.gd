...
## Handles tutorial-tab events and keeps the progress/status UI up to date.
##
## Usage – two-step initialisation:
##   1. Call setup() with service Callables.
##   2. After SettingsMenuTutorialSectionScript.build_section() returns,
##      call set_node_refs() so the Label/VBoxContainer are available.
##
## Public API for the settings menu:
##   update_progress_display() – call from update_ui_text()
##   update_status_labels()    – call wherever tutorial state may have changed
...
# ── Node refs (set in set_node_refs, after build_section returns) ─────────────
...
# ── Public refresh helpers (also called externally) ───────────────────────────

## Refreshes the "Progress: X/Y (Z%)" label.
...
## Refreshes the ✓ / Not Seen status label for every tutorial step row.
