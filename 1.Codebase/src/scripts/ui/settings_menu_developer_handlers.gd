...
## Handles developer-tab button events that mutate GameState directly.
##
## Usage – two-step initialisation:
##   1. Call setup() immediately with service Callables.
##   2. After SettingsMenuDeveloperSectionScript.build_section() returns,
##      call set_node_refs() with the result Dictionary so the spinbox
##      references are available when the user actually presses a button.
...
var _get_game_state_fn: Callable  # () -> Node|null
var _show_notification_fn: Callable  # (msg: String, success: bool) -> void
var _play_sfx_fn: Callable  # (name: String) -> void
var _report_info_fn: Callable  # (msg: String, details: Dictionary) -> void

# ── Spinbox node refs (set in set_node_refs, after build_section returns) ────
...
## Step 2 – store node refs from the build_section result Dictionary.
## Safe to call before the spinboxes are used; they are only accessed when
## the user triggers a handler, by which point _ready() has completed.
